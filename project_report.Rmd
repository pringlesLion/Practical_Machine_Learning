...
### Building a model
As this is a typical multi classfication problem, my first thought is to use a svm or softmax classifier.The first model I trained is a svm classifier with RBF kernel. 
...
Surprisingly the first try is a good start. The prediction looks good with accuracy about 86% and errors are within acceptable ranges. However I want to perfectly predict the final testing data with 100% correct, I need to improve the model. I tried to change the kernel of Therefore I chose to use another non-linear classifier. Random Forest is very powerful and might do the work.

```{r train random forest, eval=FALSE}
fitrf <- randomForest(classe~., data = pcatr)
```
...
The prediction result on validation set is surprisingly awsome to me. Let's see the random forest model.
...
### Conclusions
With the data set generated from multiple measuring instruments, we can accurately predict how well a person is preforming an excercise using a relatively simple machine learning model.The svm model has lower accuracy but has better performance than random forest.
